...
from geoseg.modules.temporal_encoders.ltae2d import LTAE2d
...
# weights_path_backbone = "/beegfs/work/y0092788/pre_train_weights/DOFA_ViT_base_e120.pth"
backbone = DOFAViTMM(img_size_lr=image_size, out_indices=out_indices, num_heads=6, embed_dims=384)
...
out_channels_decoder = 384
use_separate_necks = False
# out_shape_decoder = [image_size//4, image_size//4]
if shared_decoder:
    spatial_decoder = ConvTransformerTokensToEmbeddingNeck(384, out_channels_decoder, norm_cl=norm_cl, 
    ...
    for modality in active_modalities:
        modality_config[modality]["spatial_decoder"] = ConvTransformerTokensToEmbeddingNeck(384, out_channels_decoder, norm_cl=norm_cl, 
                                                           only_last_idx = only_last_idx, use_separate_necks=use_separate_necks)

# temporal encoder
single_temporal = False
single_temporal = False
d_model_temporal_encoder = 384
out_channels_temporal_encoder = 384
# n_head_decoding = 32
d_k = 4
n_head = 16
max_len_pe = 3000
pe_cst = 10000
mlp_channes_temporal_encoder = None
modality_config["s1"]["temporal_encoder"] = LTAE2d(in_channels=384, d_model=d_model_temporal_encoder, mlp=mlp_channes_temporal_encoder, n_head=n_head)
for modality in ("s2", "planet"):
...
# head 
head = FCNHead(in_channels=(out_channels_decoder*len(active_modalities)), num_classes=num_classes, norm=norm_cl)
