...
out_shape_planet = [(image_size*planet_ratio)//4, (image_size*planet_ratio)//4]
modality_config["planet"]["spatial_decoder"] = SegformerDecoder(out_shape_planet, in_channels=in_channel_spatial_decoder, 
                    out_channels=out_channels_decoder, norm_cl=norm_cl)
...
train_dataset = MultiModalHarzDataset(data_root, patch_size=128, mode="train", invalid_patch_policy="drop", 
                                      ...
                                      active_modalities=active_modalities, single_temporal=single_temporal, train_transform=get_training_transform, temporal_dropout=None)
