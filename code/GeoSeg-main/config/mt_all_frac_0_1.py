...
single_temporal = False
d_model_temporal_encoder = 768
out_channels_temporal_encoder = 768
# n_head_decoding = 32
d_k = 8
n_head = 32
max_len_pe = 3000
pe_cst = 10000
mlp_channes_temporal_encoder = [d_model_temporal_encoder, 64]
modality_config["s1"]["temporal_encoder"] = LTAE2d(in_channels=768, d_model=d_model_temporal_encoder, mlp=mlp_channes_temporal_encoder, n_head=n_head)
...
train_dataset = MultiModalHarzDataset(data_root, patch_size=128, mode="train", invalid_patch_policy="drop", 
                                      ...
                                      active_modalities=active_modalities, single_temporal=single_temporal, train_transform=get_training_transform, 
                                      temporal_dropout=train_temporal_dropout, data_fraction=data_fraction)
val_dataset = MultiModalHarzDataset(data_root, patch_size=128, mode="validation", invalid_patch_policy="drop", 
                                      invalid_indices_path=invalid_indices_path, indices_path=splits_path,
                                      active_modalities=active_modalities, single_temporal=single_temporal, data_fraction=data_fraction)
