...
from geoseg.modules.backbones.swin_transformer_moe import SwinTransformerMoE
...
backbone = SwinTransformerMoE(swin_modality_config)
...
modality_config["s2"]["temporal_encoder"] = MaxPoolTemporalEncoding()
...
# aux_loss = None
aux_losses = {
    "s1":nn.CrossEntropyLoss(label_smoothing=smooth_factor, ignore_index=ignore_index),
    "s2":nn.CrossEntropyLoss(label_smoothing=smooth_factor, ignore_index=ignore_index),
}
loss = TempoModalLoss(main_loss, auxiliary_losses=aux_losses)
...
def get_training_transform(modality_config):
    train_transform = [
        ...
        ToTensor(),
        RandomCropResize(crop_size=(int(image_size*crop_ratio), int(image_size*crop_ratio))),
