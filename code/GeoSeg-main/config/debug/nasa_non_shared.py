...
image_size = 128
planet_ratio = 1
planet_hw = [image_size*planet_ratio, image_size*planet_ratio]
...
backbone = DOFAViTMM(img_size_lr=image_size, img_size_hr=image_size)
...
out_channels_neck = 512
use_separate_necks = True
# out_shape_decoder = [image_size//4, image_size//4]
if shared_neck:
    spatial_decoder = ConvTransformerTokensToEmbeddingNeck(768, out_channels_neck, norm_cl=norm_cl, 
                                                           only_last_idx = only_last_idx, use_separate_necks=use_separate_necks)
    for modality in active_modalities:
        modality_config[modality]["neck"] = spatial_decoder

else:
    for modality in active_modalities:
        modality_config[modality]["neck"] = ConvTransformerTokensToEmbeddingNeck(768, out_channels_neck, norm_cl=norm_cl, 
                                                           only_last_idx = only_last_idx, use_separate_necks=use_separate_necks)
# decoder 
# decoder 
shared_decoder = True
...
out_shape_decoder = [image_size, image_size]
if shared_decoder:
...
modality_config["planet"]["temporal_encoder"] = MaxPoolTemporalEncoding()
...
def get_training_transform(modality_config):
    train_transform = [
        ...
        ToTensor(),
        # RandomCropResize(planet_ratio=1, crop_size=(int(image_size*crop_ratio), int(image_size*crop_ratio)), p=0.75),
        Normalize(modality_config)
    ]
    return Compose(train_transform)
train_temporal_dropout = TEMP_DROPOUT_PAPER
train_dataset = MultiModalHarzDataset(data_root, patch_size=128, mode="train", invalid_patch_policy="drop", planet_ratio=planet_ratio,
                                      ...
                                      train_transform=get_training_transform, temporal_dropout=TEMP_DROPOUT_PAPER)
val_dataset = MultiModalHarzDataset(data_root, patch_size=128, mode="validation", invalid_patch_policy="drop", planet_ratio=planet_ratio, 
                                      invalid_indices_path=invalid_indices_path, indices_path=splits_path,
                                      active_modalities=active_modalities, single_temporal=single_temporal)
test_dataset = MultiModalHarzDataset(data_root, patch_size=128, mode="test", invalid_patch_policy="drop", planet_ratio=planet_ratio, 
                                      invalid_indices_path=invalid_indices_path, indices_path=splits_path,
                                      active_modalities=active_modalities, single_temporal=single_temporal)
