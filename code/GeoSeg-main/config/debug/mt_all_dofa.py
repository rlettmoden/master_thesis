...
param_calculator = lambda model: sum(p.numel() for p in model.parameters())
param_calculator_trainable = lambda model: sum(p.numel() for p in model.parameters() if p.requires_grad)
...
# backbone
weights_path_backbone = "/beegfs/work/y0092788/pre_train_weights/DOFA_BASE_E122_FULLWeight.pth"
backbone = DOFAViTMM(img_size_lr=image_size)
backbone.init_weights_new(weights_path_backbone)

print(f"BACKBONE PARAMS: {param_calculator(backbone)} | TRAINABLE: {param_calculator_trainable(backbone)}")

weights = torch.load(weights_path_backbone, map_location="cpu")
...
if shared_neck:
    ...
    for modality in active_modalities:
        modality_config[modality]["neck"] = MultiLevelNeck(in_channels=in_channels_neck, 
                                                        out_channels=out_channels_neck, 
                                                        scales=scales_neck)
        print(f"NECK PARAMS: {param_calculator(modality_config[modality]["neck"])} | TRAINABLE: {param_calculator_trainable(modality_config[modality]["neck"])}")
...
if shared_decoder:
    spatial_decoder = UperNet(feature_channels=in_channel_spatial_decoder, 
                            ...
                            pool_scales=pool_scales_decoder, norm_cl=norm_cl)
    for modality in active_modalities:
    ...
    for modality in active_modalities:
        modality_config[modality]["spatial_decoder"] = UperNet(feature_channels=in_channel_spatial_decoder, 
                                                        fpn_out=out_channels_decoder, 
                                                        pool_scales=pool_scales_decoder, norm_cl=norm_cl)
        print(f"NECK PARAMS: {param_calculator(modality_config[modality]["spatial_decoder"])} | TRAINABLE: {param_calculator_trainable(modality_config[modality]["spatial_decoder"])}")
...
train_dataset = MultiModalHarzDataset(data_root, patch_size=128, mode="train", invalid_patch_policy="drop", 
                                      ...
                                      active_modalities=active_modalities, single_temporal=single_temporal, train_transform=get_training_transform, temporal_dropout=TEMP_DROPOUT_PAPER)
