...
class GLTAE2d(nn.Module):
    def __init__(
        ...
        if positional_encoding:
            self.positional_encoder = PositionalEncoder(
                self.d_model // (n_head*2), T=T, repeat=n_head
            )
        else:
            self.positional_encoder = None

        self.attention_heads = MultiHeadAttention(
            n_head=n_head, d_k=d_k, d_in=self.d_model // 2
        )
        self.lconv = LightweightConv1d(input_size=self.d_model // 2, kernel_size=3, weight_softmax=True, num_heads=n_head, padding=1)
    ...
    def forward(self, x, doy, padding_mask):#, batch_positions=None, pad_mask=None, return_comp=False):
        sits = x
        pad_mask  = None
        return_comp = False
        sz_b, seq_len, d, h, w = sits.shape
        ...
        out = sits.permute(0, 3, 4, 1, 2).contiguous().view(sz_b * h * w, seq_len, d)
        out = self.in_norm(out.permute(0, 2, 1)).permute(0, 2, 1)

        if self.inconv is not None:
            out = self.inconv(out.permute(0, 2, 1)).permute(0, 2, 1) # B, T, D -> B x D x T -> B T D
        x_ltae = out[:, :, :out.shape[2]//2]
        x_convs = out[:, :, out.shape[2]//2:]
        if self.positional_encoder is not None:
            ...
            x_ltae = x_ltae + self.positional_encoder(bp)

        # ltae
        x_ltae, attn = self.attention_heads(x_ltae, pad_mask=pad_mask)
        x_ltae = (
            x_ltae.permute(1, 0, 2).contiguous().view(sz_b * h * w, -1)
        )  # Concatenate heads
        # convs
        # x_convs = self.lconv(x_convs.reshape(seq_len, sz_b * h * w, out.shape[2]//2))
        x_convs = self.lconv(x_convs.reshape(sz_b * h * w, out.shape[2]//2, seq_len)).permute(2, 0, 1) # B T C -> B C T -> T B C
        x_convs = torch.mean(x_convs, dim=0) # average pool T dimension

        out = torch.concat([x_ltae, x_convs], dim=1)
        ...
        temporal_out_dict["features"] = out

        if self.return_att:
            temporal_out_dict["attention_mask"] = attn
