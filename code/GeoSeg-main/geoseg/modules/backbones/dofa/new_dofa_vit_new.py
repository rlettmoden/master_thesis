...
class DOFAViTMM(nn.Module):
    ...
    def init_weights_new(self, checkpoint_path):
        ...
        if 'pos_embed' in state_dict.keys():
            ...
            # if self.img_size_hr != self.img_size_lr:
            #     init_pos_emb(img_size=self.img_size_lr, pos_embed_module=self.pos_embed_hr)
    ...
    def forward(self, inputs):
        raise NotImplementedError("Use forward_modality instead")
        # for modality in self.active_modalities:
        #     # forward U-BARN
        #     # BxTxCxHxW -> B*TxCxHxW
        #     input = inputs[modality]
        #     apply_reshape = len(input.shape) == 5
        #     if apply_reshape:
        ...
        #     out = self.forward_modality(out, modality)
