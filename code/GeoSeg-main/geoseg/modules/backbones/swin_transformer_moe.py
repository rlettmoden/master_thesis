...
class SwinTransformerMoE(nn.Module):
    ...
    def __init__(self, modalities, img_size=224, patch_size=16, in_chans=3,
        ...
        self.global_experts = num_local_experts * 4 # TODO avoid HARD CODED 
        print(f"N experts: {self.global_experts}")
        self.sharded_count = (1.0 / num_local_experts) if num_local_experts > 0 else (-num_local_experts)

        # split image into non-overlapping patches
        # Create a PatchEmbed module for each modality
        self.patch_embeds = nn.ModuleDict()
        for modality_key, modality_config in modalities.items():
            self.patch_embeds[modality_key] = PatchEmbed(
                img_size=modality_config['image_size'],
                patch_size=patch_size,
                in_chans=modality_config['in_chans'],
                embed_dim=embed_dim,
                norm_layer=norm_layer if self.patch_norm else None
            )

        # Create absolute position embeddings for each modality
        if self.ape:
            self.absolute_pos_embeds = nn.ParameterDict()
            for modality_key, modality_config in self.modalities.items():
                num_patches = self.patch_embeds[modality_key].num_patches
                self.absolute_pos_embeds[modality_key] = nn.Parameter(torch.zeros(1, num_patches, embed_dim))
                trunc_normal_(self.absolute_pos_embeds[modality_key], std=self.init_std)
    ...
    def forward_modality(self, x, modality_key):
        modality_config = self.modalities[modality_key]
        
        # Use the modality-specific patch embedding
        x = self.patch_embeds[modality_key](x)
        
        # Apply absolute position embedding if enabled
        if self.ape:
            x = x + self.absolute_pos_embeds[modality_key]
